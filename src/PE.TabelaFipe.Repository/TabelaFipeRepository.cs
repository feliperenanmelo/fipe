...
using PE.TabelaFipe.Domain.Models.Exceptions;
...
namespace PE.TabelaFipe.Repository
{
    public class TabelaFipeRepository : ITabelaFipeRepository
    {
        ...
        public async Task<IEnumerable<Marca>> ObterMarcas(string marca)
        {
            ...
            {
                var marcasViewModel = JsonConvert.DeserializeObject<IEnumerable<Marca>>(conteudo);
                return marcasViewModel;
            }

            throw new RequestErrorException(conteudo, response.StatusCode,conteudo);
        }
        ...
        public async Task<IEnumerable<Modelo>> ObterModelos(string marca, int codigoMarca)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var dadosModeloView = JsonConvert.DeserializeObject<DadosModelo>(conteudo);
        ...
        public async Task<IEnumerable<Modelo>> ObterModelosPorAno(string marca, int codigoMarca, int codigoModelo)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var modelosPorAno = JsonConvert.DeserializeObject<IEnumerable<Modelo>>(conteudo);
        ...
        public async Task<Fipe> ObterPreco(string marca, int codigoMarca, int codigoModelo, string codigoAno)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var tabelafipeVielModel = JsonConvert.DeserializeObject<Fipe>(conteudo);
